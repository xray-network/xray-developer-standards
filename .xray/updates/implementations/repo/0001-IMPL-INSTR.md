...
| `C02` | Create the aggregate status ledger and monorepo implementation directories, including reserved target `repo`. | Do not create target-local status ledgers or mix flat records with target directories. | Repository governance | Inspect the installed layout and ledger links. |
...
- `git diff --check`
- Confirm the aggregate ledger contains exactly one `repo/0001` row with matching instruction and
  result links.
...
- The aggregate ledger records `repo/0001` as `ACCEPTED` with the required decision proof.
